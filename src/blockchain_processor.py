...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.lanacoind_url = 'http://%s:%s@%s:%s/' % (
            config.get('lanacoind', 'lanacoind_user'),
            config.get('lanacoind', 'lanacoind_password'),
            config.get('lanacoind', 'lanacoind_host'),
            config.get('lanacoind', 'lanacoind_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.lanacoind('getblock', [self.storage.last_hash]))
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("lanacoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def lanacoind(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.lanacoind_url, postdata).read()
                break
            except:
                print_log("cannot reach lanacoind...")
                self.shared.pause()
                time.sleep(10)
    ...
    def get_header(self, height):
        block_hash = self.lanacoind('getblockhash', [height])
        b = self.lanacoind('getblock', [block_hash])
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.lanacoind('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.lanacoind_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            try:
                txo = self.lanacoind('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.lanacoind('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.lanacoind('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.lanacoind('getblock', [block_hash])
        ...
        try:
            ...
            logger.error("lanacoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run lanacoind with txindex=1; use -reindex if needed.")
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.lanacoind('getinfo')
            self.lanacoind_height = info.get('blocks')
            lanacoind_block_hash = self.lanacoind('getblockhash', [self.lanacoind_height])
            if self.storage.last_hash == lanacoind_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.lanacoind('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.lanacoind('getrawmempool'))
